...
class TransactionService {
  ...
  Future<List<dynamic>> getTransactionHistory() async {
    final token = await _apiService.getToken();
    debugPrint("Token for transaction history: ${token != null ? 'exists' : 'null'}");
    ...
    try {
      ...
      if (!response.containsKey("data")) {
        ...
        if (response is Map<String, dynamic> && response.containsKey("transactions")) {
          debugPrint("Found transactions key in response");
          result = response["transactions"] ?? [];
        }
        
        // If we have an array directly in the response
        else if (response is Map<String, dynamic> && response.containsKey("content") && response["content"] is List) {
        ...
        else if (response is Map<String, dynamic> && response.values.any((v) => v is List)) {
        ...
        debugPrint("Transaction History Data: $data (type: ${data.runtimeType})");
  ...
  Future<List<dynamic>> filterTransactions({
    ...
    final token = await _apiService.getToken();
    debugPrint("Token for filter transactions: ${token != null ? 'exists' : 'null'}");
    ...
    if (!response.containsKey("data")) {
      ...
      if (response is Map<String, dynamic> && response.containsKey("content") && response["content"] is List) {
        debugPrint("Found content key with list in response");
        return response["content"] ?? [];
      }
      
      // Try to determine if the response itself is the list of transactions
      if (response is Map<String, dynamic> && response.values.any((v) => v is List)) {
        for (var key in response.keys) {
          if (response[key] is List) {
            debugPrint("Found list in response at key: $key");
            return response[key];
  ...
  Future<List<dynamic>> searchTransactionHistory(String query) async {
    final token = await _apiService.getToken();
    debugPrint("Token for transaction search: ${token != null ? 'exists' : 'null'}");
    ...
    try {
      final response = await _apiService.get(
        ...
        token: token,
        queryParams: params  // Ensure this matches the parameter name in _apiService.get method
      );
